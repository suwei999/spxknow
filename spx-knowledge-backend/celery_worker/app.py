...
- Auto-start Celery worker in background when running `python app.py`
"""
...
_worker_proc = None  # 全局保存子进程句柄，便于优雅退出
...
async def lifespan(app: FastAPI):
    ...
    if os.getenv("CELERY_AUTOSTART", "true").lower() == "true":
        try:
            ...
            # 以子进程方式启动，将工作目录设置为项目根，保证 `-m celery_worker.worker` 可导入
            ...
            logger.error(f"内嵌 Celery Worker 启动失败: {e}", exc_info=True)
